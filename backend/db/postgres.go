...
func Connect(databaseURL string) error {
	...
	// ✅ CRITICAL: Disable prepared statements for Transaction pooler
	...
	log.Printf("   MaxConns: %d, MinConns: %d, Port: 6543, SimpleProtocol: YES", config.MaxConns, config.MinConns)
	...
	if wsURL != "" {
		...
		if err != nil {
			...
			// ✅ Session pool: Keep default QueryExecMode (allows prepared statements)
			// No need to set DefaultQueryExecMode - default is fine for Session mode
			...
			if err != nil {
				...
				if err := pingWithRetry(WSPool, 3); err != nil {
					...
					log.Printf("   MaxConns: %d, MinConns: %d, Port: 5432, PreparedStatements: YES", wsConfig.MaxConns, wsConfig.MinConns)
				}
			}
		}
	} else {
